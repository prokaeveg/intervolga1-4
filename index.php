...
<html>
 ...
 <body>
	...
	<?php
		...
		echo "<b>Полный текст новости в переменной а:</b><br> ", $a;

		$b = trim(substr($a, 0, 180));
		$exp = explode(" ",$b);
		$max = max(array_keys($exp));
		$repl = $exp[$max-1]." ".$exp[$max];
		echo "<br><br><b>Итоговый текст в переменной c: </b><br>";
		echo preg_replace("/($repl)/i", "<a href='$link'>\\1...</a>", $b);
	?>
	...
	<p><b>Ответ: </b><br>По сути, эти два запроса ничем не отличаются, результат их работы будет одинаковый, но если представить себе запрос из 4-5 связываний и где условия прописаны в секции where и там же будут прописаны условия фильтрации, не входящие в условия связывания, такой запрос невозможно прочитать. Поэтому второй вариант лучше с точки зрения стиля и удобства понимания кода</p>
	...
	<?php
		...
		echo "<p><b>Пар одинаковых элементов: </b></p>",$count;
	?>
 </body>
 </html>